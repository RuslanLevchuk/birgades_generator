...
#запит на отримання нзви робочого центру, складу бригади згідно наданої назви асортименту
...
#запит на отримання переліку робочих центрів
...
#запит на отримання переліку бригад
...
#Запит на перевірку існування асортименту у БД, повертається рядковий тип, який опрацьовується у тілі основної програми
def sqlite_is_assortment_exist(name):
    ...
    # ящо існує асортимент у таблиці, то повертається рядок про існування найменування
    if name in [i[0] for i in assortments_respond]:
        return 'Такий асортимент у базі даних уже існує!'
    #ящо ще нічого не прийшло у запит, то повертається рядок про необхідність введення назви асортименту
    elif name == '':
        return 'Введіть назву асортименту'
    # якщо не існує найменування у БД, то повертається пустий рядок
    else:
        return ''

# запит на збереження асортименту у БД
...
if __name__ == '__main__':
    ...
    print(sqlite_is_assortment_exist(
        'ШОКОЛАД Lacmi молочний з цілими лісовими горіхами і шоколао-карамельною начинкою ВКФ 295г /12шт'))
